...
if [ $tgt_l == "de" ]; then
  agent=${SCRIPT_ROOT}/simulst/agents/v1.1.0/s2t_alignatt_word.py
  sacrebleu_tokenizer=13a
elif [ $tgt_l == "ja" ]; then
  agent=${SCRIPT_ROOT}/simulst/agents/v1.1.0/s2t_alignatt_char.py
...
simuleval_log_dir_name=${ts}.${ckpt}.${who}.${tgt_l}.alignatt.chunk${chunk_size}-beam${beam}-frame${frame_num}_ca
