...
class SimulSTBaseAgent(SpeechAgent):

    def __init__(self, args):
        super().__init__(args)

        self.speech_segment_size = args.chunk_size
    ...
    def units_to_segment(self, units, states):
        ...
        if self.args.eval_latency_unit == "char":
    ...
    def predict(self, states):
        ...
        # specify target language id for mBART prefix
        if states.target.length() == 0 and self.args.lang is not None:
            if self.args.lang == "de":
                index = self.model.decoder.dictionary.index("<lang:de>")
            elif self.args.lang == "ja":
                index = self.model.decoder.dictionary.index("<lang:ja>")
            elif self.args.lang == "zh":
                index = self.model.decoder.dictionary.index("<lang:zh>")
