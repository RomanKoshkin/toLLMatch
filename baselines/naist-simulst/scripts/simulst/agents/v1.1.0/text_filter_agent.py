...
class LongTextFilterAgent(SimpleTextFilterAgent):
    """
    Simply filter out the some tokens in the output.
    """
    ...
    def policy(self) -> Action:
        ...
        for i in self.args.filtered_tokens:
            self.states.source[-1] = self.states.source[-1].replace(i, "")
        return WriteAction(self.states.source[-1], finished=self.states.source_finished)
