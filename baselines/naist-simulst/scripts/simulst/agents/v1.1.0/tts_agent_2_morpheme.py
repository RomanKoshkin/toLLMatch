...
class Subword2Yomi2SpeechAgent(TextToSpeechAgent):
    ...
    def policy(self) -> Action:
        if len(self.states.source) == 0:
            ...
            # TTS with morpheme unit + minimal length = self.states.input_minimal_length
            if not yomi_out:
                return ReadAction()
            elif ( len(yomi_out) + len(self.states.tts_input_chunks) ) < self.args.tts_input_minimal_length and not "</s>" in yomi_out:
              self.states.tts_input_chunks += yomi_out
              return ReadAction()
            else:
              yomi_out = self.states.tts_input_chunks + yomi_out
              self.states.tts_input_chunks = []
            ...
            tts_inputs = torch.from_numpy(self.states.model2.text2seq(yomi_out))\
                .unsqueeze(0).long().to(self.states.devices)
            
            if self.states.is_decode_first and not self.states.source_finished:
                self.states.out_pos.append(tts_inputs.shape[-1])
                self.states.cat_inputs=tts_inputs
                ...
                self.states.out_pos.append(
                    tts_inputs.shape[-1] if len(self.states.out_pos) == 0
                    else self.states.out_pos[-1] + tts_inputs.shape[-1]
                )
                memory2 = self.states.model2.encode(
                    tts_inputs if self.states.cat_inputs is None 
                    else torch.cat( (self.states.cat_inputs,tts_inputs), dim=1)
                )
                self.states.cat_inputs = None

            if self.states.model2.eos_seq in tts_inputs:
                ...
                mel_out, post_mel_out, acct, stop = self.states.model2.decode(
                    memory2, self.states.mel_inputs, 
                    self.states.out_pos[0] if len(self.states.out_pos) != 0
                    else -1
                )
                if len(self.states.out_pos)!=0:
                  self.states.out_pos.pop(0) 
                wav = self.states.model3.synthesis(post_mel_out)
